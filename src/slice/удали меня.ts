...
describe('тесты синхронных экшенов', () => {
  ...
  test('тест редюсера конструтора сортировка', () => {
    // ставим лайк с помощью экшена toggleLike

    const newState = constructorReducer(
      { ingredients: mockIngredients, isLoading: true },
      sortIngredients({ to: 0, from: 1 })
    );
...
describe('getIngredients async action', () => {
  ...
  test('should fetch ingredients and store them correctly', async () => {
    // Мокаем API с mockImplementation
    jest
      .spyOn(api, 'getIngredients')
      .mockImplementation(() =>
        Promise.resolve({ ingredients: mockIngredients })
      );
